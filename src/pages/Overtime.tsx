...
import { Plus, Trash2, IndianRupee, Calendar, Clock } from 'lucide-react';
import type { Employee, Overtime as OvertimeType } from '../types';
...
export default function Overtime() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setFormData({ ...formData, hours: 0, rate: 0 });
    fetchData();
  };
    ...
    >
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-2">Log extra hours and calculate overtime pay.</p>
        </div>
        <button
          onClick={() => setIsModalOpen(true)}
          className="bg-purple-600 hover:bg-purple-700 text-white px-6 py-3 rounded-xl font-medium shadow-lg shadow-purple-500/30 transition-all flex items-center gap-2 premium-glossy"
        >
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/50 text-slate-500 text-sm uppercase tracking-wider">
                ...
                <th className="px-6 py-4 font-medium">Rate / Hr</th>
              ...
              {loading ? (
                ...
                overtime.map((ot) => (
                  <tr key={ot.id} className="hover:bg-slate-50/50 transition-colors group">
                    ...
                    <td className="px-6 py-4">
                      <div className="font-semibold text-slate-900">{ot.full_name}</div>
                      <div className="text-xs text-slate-500">ID: #{ot.employee_id}</div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-1 text-slate-700 font-medium">
                        <Clock size={16} className="text-purple-500" /> {ot.hours} hrs
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="text-slate-600 flex items-center gap-1">
                        <IndianRupee size={14} /> {ot.rate}
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="font-bold text-purple-700 flex items-center gap-1">
                        <IndianRupee size={16} /> {(ot.hours * ot.rate).toLocaleString()}
                      </div>
                    </td>
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          ...
          >
            <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              <h2 className="text-xl font-bold text-slate-900">Log Overtime</h2>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                ...
                <select
                  ...
                  className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-purple-500/50 focus:border-purple-500"
                >
              ...
              <div className="p-4 bg-purple-50 rounded-xl border border-purple-100 mt-4">
                <div className="flex justify-between items-center text-sm">
                  ...
                  <span className="text-purple-900 font-bold text-lg">₹{(formData.hours * formData.rate).toLocaleString()}</span>
                </div>
              </div>
              ...
              <div className="pt-6 flex justify-end gap-3 border-t border-slate-100 mt-6">
                ...
                <button
                  type="submit"
                  className="bg-purple-600 hover:bg-purple-700 text-white px-6 py-2.5 rounded-xl font-medium shadow-lg shadow-purple-500/30 transition-all premium-glossy"
                >
                  Save Entry
                </button>
              </div>
