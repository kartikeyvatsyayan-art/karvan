...
export default function Layout() {
  ...
  const handleDbRestore = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!confirm('WARNING: This will replace the entire database. All current data will be lost. Continue?')) {
      ...
      >
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                className={twMerge(
                  clsx(
                    'flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 group relative premium-glossy',
                    isActive 
        ...
        <div className="p-4 space-y-2 border-t border-slate-800">
          <button 
            ...
            className={twMerge(
              clsx(
                "flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white py-3 rounded-xl transition-all text-sm font-medium shadow-lg shadow-emerald-900/20 overflow-hidden premium-glossy",
          ...
          >
            ...
            {!isSidebarCollapsed && <span className="whitespace-nowrap">Data Sync & Backup</span>}
          </button>
          
          <div className={twMerge(clsx("grid gap-2", isSidebarCollapsed ? "grid-cols-1" : "grid-cols-2"))}>
            <button 
              onClick={handleBackup}
              className={twMerge(
                clsx(
                  "flex items-center justify-center gap-2 bg-slate-800 hover:bg-slate-700 text-slate-300 py-3 rounded-xl transition-all text-sm font-medium overflow-hidden premium-glossy",
                  isSidebarCollapsed ? "w-12 h-12" : "w-full"
                )
              )}
              title="Backup JSON"
            >
              ...
              {!isSidebarCollapsed && <span className="whitespace-nowrap">Backup</span>}
            </button>
            <button 
              ...
              className={twMerge(
                clsx(
                  "flex items-center justify-center gap-2 bg-slate-800 hover:bg-slate-700 text-slate-300 py-3 rounded-xl transition-all text-sm font-medium disabled:opacity-50 overflow-hidden premium-glossy",
                  isSidebarCollapsed ? "w-12 h-12" : "w-full"
                )
              )}
              title="Restore JSON"
            >
              {isRestoring ? (
                ...
                <>
                  ...
                  {!isSidebarCollapsed && <span className="whitespace-nowrap">Restore</span>}
                </>
              )}
            </button>
          </div>
            ...
            >
              <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                <h2 className="text-xl font-bold text-slate-900 flex items-center gap-2">
                  <FileSpreadsheet className="text-emerald-600" /> Data Sync & Backup
                </h2>
                <button onClick={() => setIsSyncModalOpen(false)} className="text-slate-400 hover:text-slate-600 premium-glossy p-1 rounded-full">
                  <X size={20} />
                </button>
              </div>
              ...
              <div className="p-6 space-y-6">
                <div className="space-y-2">
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 bg-slate-100 hover:bg-slate-200 text-slate-700 py-3 rounded-xl transition-colors font-medium border border-slate-200 premium-glossy"
                  >
                ...
                <div className="space-y-2">
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 py-3 rounded-xl transition-colors font-medium border border-emerald-200 disabled:opacity-50 premium-glossy"
                  >
                    {isUploading ? (
                ...
                <div className="relative py-2">
                  ...
                  <div className="relative flex justify-center text-sm">
                    <span className="px-2 bg-white text-slate-400">FULL DATABASE</span>
                  </div>
                </div>

                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-2">
                    <h3 className="font-semibold text-slate-900 text-sm">Full Backup</h3>
                    <button
                      onClick={handleDbBackup}
                      className="w-full flex items-center justify-center gap-2 bg-slate-100 hover:bg-slate-200 text-slate-700 py-2.5 rounded-xl transition-colors text-xs font-bold border border-slate-200 premium-glossy"
                    >
                      <Download size={14} /> Download .db
                    </button>
                  </div>
                  <div className="space-y-2">
                    <h3 className="font-semibold text-slate-900 text-sm">Full Restore</h3>
                    <input 
                      type="file" 
                      accept=".db" 
                      className="hidden" 
                    ...
                    <button
                      ...
                      className="w-full flex items-center justify-center gap-2 bg-red-50 hover:bg-red-100 text-red-700 py-2.5 rounded-xl transition-colors text-xs font-bold border border-red-200 disabled:opacity-50 premium-glossy"
                    >
                      {isDbRestoring ? (
